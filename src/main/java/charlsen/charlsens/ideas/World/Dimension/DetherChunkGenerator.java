...
public class DetherChunkGenerator extends ChunkGenerator {

    public static void registerChunkgenerator() {
        Registry.register(Registry.CHUNK_GENERATOR, new Identifier(Charlsensideas.MODID, "dether_chunk_generator"), DetherChunkGenerator.CODEC);
    }

    public static final Codec<DetherChunkGenerator> CODEC = RecordCodecBuilder.create(
            ...
            ).apply(instance, instance.stable(DetherChunkGenerator::new)));
    ...
    private static final SpawnSettings.SpawnEntry INITIAL_HONEY_SLIME_ENTRY = new SpawnSettings.SpawnEntry(EntityType.SLIME, 1, 1, 3);
    ...
    public DetherChunkGenerator(BiomeSource biomeSource, StructuresConfig structureConfig) {
        this(biomeSource, biomeSource, structureConfig);
    }

    private DetherChunkGenerator(BiomeSource biomeSource, BiomeSource biomeSource2, StructuresConfig structureConfig) {
        ...
        this.defaultFluid = CharlsensideasBlocks.Weird_Water.getDefaultState();
    ...
    public ChunkGenerator withSeed(long seed) {
        return new DetherChunkGenerator(this.biomeSource.withSeed(seed), this.structureConfig);
    }
    ...
    protected void makeCeilingAndFloor(Chunk chunk, Random random) {
        ...
        for (BlockPos blockpos : BlockPos.iterate(xStart, 0, zStart, xStart + 15, 0, zStart + 15))
        {
            ...
            {
                chunk.setBlockState(blockpos$Mutable.set(blockpos.getX(), ceilingY, blockpos.getZ()), CharlsensideasBlocks.CloudBlock.getDefaultState(), false);
            }

            //single layer of solid blocks
            for (int floorY = 0; floorY <= floorHeight + random.nextInt(2); ++floorY)
            {
                chunk.setBlockState(blockpos$Mutable.set(blockpos.getX(), floorY, blockpos.getZ()), CharlsensideasBlocks.CloudBlock.getDefaultState(), false);
            }
        }
    }
